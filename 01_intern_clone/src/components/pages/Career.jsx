...
const Career = () => {
      ...
      <div className="relative z-10 mt-20 max-w-7xl w-full px-4">
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-8 mt-10">
          {stats.map((item, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="flex flex-col items-center justify-center w-64 p-6 rounded-2xl bg-[#1e1e1e] shadow-md"
            >
              <h1 className="text-[#319c1c] text-3xl font-semibold">{item.number}</h1>
              <p className="text-gray-400 font-semibold mt-2 text-sm text-center">{item.label}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-16">
          {domainCards.map((card, index) => (
            <div
              key={index}
              className="flex flex-col items-start border border-gray-700 p-6 space-y-4 rounded-2xl bg-[#1e1e1e] shadow-lg hover:scale-105 transition-all duration-300"
            >
              <div className="flex items-center justify-center w-14 h-14 rounded-xl bg-[#002f06]">
                {card.icon}
              </div>
              <h2 className="text-white text-2xl font-semibold w-full">{card.title}</h2>
              <p className="text-gray-400 text-sm w-full">{card.description}</p>
              <ul className="text-gray-400 space-y-2 w-full">
  ...
  <div className='flex gap-6 w-[100rem]'>
    <div className="flex flex-col border border-gray-700 p-6 w-[52rem] space-y-4 rounded-2xl bg-[#1e1e1e] shadow-lg">
            ...
            {learningPhases.map((phase, idx) => (
              <div key={idx} className="w-full">
                ...
                <div className="bg-gray-800 rounded-full h-2 w-full">
                  <div className="bg-green-500 h-2 rounded-full" style={{ width: `${phase.progress}%` }}></div>
                </div>
              </div>
            ))}
          </div>

          <div className="flex flex-col border border-gray-700   p-6 space-y-4 rounded-2xl bg-[#1e1e1e] shadow-lg">
        ...
        <div className="mt-20 flex justify-center">
          <div className="bg-[#1e1e1e] text-center px-20 py-30 rounded-2xl w-full max-w-6xl shadow-lg">
